                                                                            ...
                                                                            origin : input : sed : ignore :
                                                                                {
                                                                                    ...
                                                                                    init-text =
                                                                                        ''
                                                                                            ...
                                                                                            git checkout $COMMIT
                                                                                            ${ if sed then "sed -i ${ builtins.concatStringsSep " " ( builtins.attrValues ( builtins.mapAttrs ( name : value : ''-E 's/([[:alnum:]_]+\.url[[:space:]]*=[[:space:]]*")([^"]+)(")/\1\2?ref=XXXX\3/''' ) ) ) resources.milestone.source.inputs } $SELF/work-tree/flake.nix" else "# INPUT FLAKE" }
                                                                                        '' ;
                                                                                } ;
                                                                        in
                                                                            {

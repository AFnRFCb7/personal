                                    ...
                                    let
                                        ...
                                        resources_ =
                                            let
                                                                                                                ...
                                                                                                                do
                                                                                                                    case "$1" in
                                                                                                                        --flake)
                                                                                                                            ...
                                                                                                                            REPOSITORY="$( ${ resources._milestone.repository } "$REMOTE" "$MILESTONE" "$SCRATCH" "$BRANCH" "$COMMIT" )" || exit 64
                                                                                                                            echo AFTER >> "$DIR/flag"
							                    ...
							                    in visitor.lib.implementation { lambda = path : value : resources.lib.implementation ( { nixpkgs = nixpkgs ; path = path ; secret-directory = "/home/${ config.personal.name }/resources" ; seed = [ nixpkgs path private self secret system visitor ] ; system = system ; } // ( value path ) ) ; } tree ;
                                        in
                                            {
                                                config =
                                                    {
                                                                    ...
                                                                    let
									                                    ...
									                                    in
                                                                            [
                                                                                ...
                                                                                ( studio "studio-resources" resources_.repository.secret )

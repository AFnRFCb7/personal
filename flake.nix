                                                                    ...
                                                                    ignore :
                                                                        {
                                                                            ...
                                                                            init-text =
                                                                                ''
                                                                                    ...
                                                                                    cd "$( ${ resources.milestone.source.private } "$1" )/work-tree"
                                                                            ...
                                                                            origin : input-script : sed : ignore :
                                                                                {
                                                                                    ...
                                                                                    init-text =
                                                                                        ''
                                                                                            ...
                                                                                            ${ if sed then ''sed -i ${ builtins.concatStringsSep " " ( builtins.attrValues ( builtins.mapAttrs ( name : value : ''-e "s#\(${ name }.url.*?ref=\)main\(\".*\)\$#\1$( GIT_DIR=$( ${ value } )/git GIT_WORK_TREE=$( ${ value } )/work-tree ${ pkgs.git }/bin/git rev-parse HEAD )\2#"'' ) resources.milestone.source.inputs ) ) } "${ builtins.concatStringsSep "" [ "$" "{" "SED_EXPR[@]" "}" ] }" "$GIT_WORK_TREE/flake.nix"'' else "# " }
                                                                                        '' ;
                                                                                } ;
                                                                        in
                                                                            {

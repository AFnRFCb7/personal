                                    ...
                                    let
                                        ...
                                        resources_ =
                                            let
							                    ...
							                    in visitor.lib.implementation { lambda = path : value : resources.lib.implementation ( { buildFHSUserEnv = pkgs.buildFHSUserEnv ; coreutils = pkgs.coreutils ; flock = pkgs.flock ; gnutar = pkgs.gnutar ; jq = pkgs.jq ; nix = pkgs.nix ; resources-directory = "/home/${ config.personal.name }/resources" ; seed = [ nixpkgs path private resources self system visitor ] ; writeShellApplication = pkgs.writeShellApplication ; yq-go = pkgs.yq-go ; zstd = pkgs.zstd ; } // ( value path ) ) ; } tree ;
                                        in
                                            {
                                                config =
                                                    {
